...
#include "video_core/renderer_metal/mtl_instance.h"
...
MICROPROFILE_DEFINE(Metal_VS, "Vulkan", "Vertex Shader Setup", MP_RGB(192, 128, 128));
MICROPROFILE_DEFINE(Metal_GS, "Vulkan", "Geometry Shader Setup", MP_RGB(128, 192, 128));
MICROPROFILE_DEFINE(Metal_Drawing, "Vulkan", "Drawing", MP_RGB(128, 128, 192));
...
void RasterizerMetal::LoadDiskResources(const std::atomic_bool& stop_loading,
                                        const VideoCore::DiskResourceLoadCallback& callback) {
    pipeline_cache.LoadDiskCache();
}

void RasterizerMetal::SyncFixedState() {
    SyncCullMode();
    SyncBlendEnabled();
    SyncBlendFuncs();
    SyncBlendColor();
...
void RasterizerMetal::SetupVertexArray() {
...
void RasterizerMetal::SetupFixedAttribs() {
    const auto& vertex_attributes = regs.pipeline.vertex_attributes;
    VertexLayout& layout = pipeline_info.vertex_layout;
...
bool RasterizerMetal::SetupVertexShader() {
    MICROPROFILE_SCOPE(Metal_VS);
    return pipeline_cache.UseProgrammableVertexShader(regs, pica.vs_setup,
                                                      pipeline_info.vertex_layout);
}

bool RasterizerMetal::SetupGeometryShader() {
    MICROPROFILE_SCOPE(Metal_GS);
...
bool RasterizerMetal::AccelerateDrawBatch(bool is_indexed) {
    if (regs.pipeline.use_gs != Pica::PipelineRegs::UseGS::No) {
...
bool RasterizerMetal::AccelerateDrawBatchInternal(bool is_indexed) {
    if (is_indexed) {
        SetupIndexArray();
    }
...
void RasterizerMetal::SetupIndexArray() {
...
bool RasterizerMetal::Draw(bool accelerate, bool is_indexed) {
    MICROPROFILE_SCOPE(Metal_Drawing);
...
void RasterizerMetal::SyncTextureUnits(const Framebuffer* framebuffer) {
...
void RasterizerMetal::SyncUtilityTextures(const Framebuffer* framebuffer) {
    const bool shadow_rendering = regs.framebuffer.IsShadowRendering();
    if (!shadow_rendering) {
        return;
    }
...
void RasterizerMetal::BindShadowCube(const Pica::TexturingRegs::FullTextureConfig& texture,
...
void RasterizerMetal::BindTextureCube(const Pica::TexturingRegs::FullTextureConfig& texture,
                                      vk::DescriptorSet texture_set) {
    using CubeFace = Pica::TexturingRegs::CubeFace;
...
void RasterizerMetal::NotifyFixedFunctionPicaRegisterChanged(u32 id) {
    switch (id) {
    // Culling
...
bool RasterizerMetal::AccelerateDisplayTransfer(const Pica::DisplayTransferConfig& config) {
    return res_cache.AccelerateDisplayTransfer(config);
}

bool RasterizerMetal::AccelerateTextureCopy(const Pica::DisplayTransferConfig& config) {
    return res_cache.AccelerateTextureCopy(config);
}

bool RasterizerMetal::AccelerateFill(const Pica::MemoryFillConfig& config) {
    return res_cache.AccelerateFill(config);
}

bool RasterizerMetal::AccelerateDisplay(const Pica::FramebufferConfig& config,
...
void RasterizerMetal::MakeSoftwareVertexLayout() {
...
void RasterizerMetal::SyncColorWriteMask() {
...
void RasterizerMetal::SyncStencilWriteMask() {
...
void RasterizerMetal::SyncDepthWriteMask() {
...
void RasterizerMetal::SyncStencilTest() {
...
void RasterizerMetal::SyncDepthTest() {
...
void RasterizerMetal::SyncAndUploadLUTsLF() {
...
void RasterizerMetal::SyncAndUploadLUTs() {
...
void RasterizerMetal::UploadUniforms(bool accelerate_draw) {
